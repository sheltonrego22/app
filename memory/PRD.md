...
## BRAND REDESIGN (Revised Brand Guidelines, Sept 2026) — PHASE 1 COMPLETE (iteration 30: all flows pass)
User decisions: Montserrat headings + Roboto body (Nobel substitute); light-first, dark/light toggle REMOVED; add Bus & Coach brand (Royal Limousine Coaches), fold Emirates Taxi into Chauffeur; new IA Home · About · Our Brands · For Business · For You · Contact + persistent "Get a Quote"; Careers/Media/Partner/FAQ/Downloads/Portals → footer + About/Support sub-menus; mobile accordion menu + sticky Call/WhatsApp/Quote bar; UAE-specific imagery from egmg.ae + brochure + user uploads; phased delivery.
Design blueprint: /app/design_guidelines.json (tokens, type scale, buttons, chips, cards, header/mega-menu, mobile, footer, homepage layout, RTL notes).
- [x] Design system: `index.css` (light tokens, Montserrat/Roboto/Cairo, LEGACY BRIDGE remapping dark utility classes to light for un-migrated pages, admin keeps dark chrome), `App.css` (btn-primary/secondary/outline/ghost, chip-orange/chip-outline, card-light, diagonal devices), tailwind brand colours + fontFamily
- [x] Navigation (`Navigation.js`, `nav/NavParts.js`, `nav/NavDropdown.js` mega menus with 7 brand tiles, `nav/MobileMenu.js` accordion, `MobileContactBar.js` 3-button sticky bar), Footer (4 columns + contact + social), IA in `i18n/navData.js` (EN + AR)
- [x] Homepage rewritten (`pages/HomePage.js`, `components/home/HeroSection.js`, `CoreSections.js`, `TrustSectionsV2.js`, `data/home.js`): hero video (egmg.ae) + WE MOVE YOU chip + dual CTAs, stats band, 7-brand grid, For You / For Business split, Steps 01–04, Why Eurogulf (dark), client logos, latest CMS news, sustainability teaser, orange final CTA. ~7,000px (was ~8,900px)
- [x] New Bus & Coach page `pages/BusCoachPage.js` at `/royal-limousine` (user's 4 coach/fleet/control-room images + brochure content), SEO entry added; `/emirates-taxi` → Chauffeur page

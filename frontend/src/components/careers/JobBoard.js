        ...
        {items.map(({ id, posted, job }) => (
          <div key={id} data-testid={`latest-job-${id}`} className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-black border border-white/5 p-4">
            ...
            <div className="flex gap-2 flex-shrink-0">
              <button onClick={() => onApply(job[lang].title)} data-testid={`latest-apply-${id}`} className="inline-flex items-center gap-1 bg-[#EE5A01] text-black font-heading font-bold text-[10px] px-3 py-2 hover:bg-[#F17B34]"><FileText className="w-3 h-3" /> {c.applyOnline}</button>
      ...
      {expanded && (
        <div className="px-5 pb-5 border-t border-white/5 pt-4">
          ...
          <div className="flex flex-wrap gap-3">
            <button onClick={() => onApply(j.title)} data-testid={`apply-online-${job.id}`} className={`inline-flex items-center gap-2 bg-[#EE5A01] text-black font-heading font-bold text-xs px-5 py-2.5 hover:bg-[#F17B34] transition-colors ${isAr ? '' : 'tracking-[0.05em]'}`}>
...
export function JobBoard({ lang = 'en' }) {
  ...
  return (
    <div data-testid={`${isAr ? 'ar-' : ''}job-board`}>
      ...
      <LatestJobs lang={lang} onApply={setApplyRole} />
      ...
      <div className="space-y-3">
        {filtered.map((job) => <JobCard key={job.id} job={job} lang={lang} expanded={expanded === job.id} onToggle={() => setExpanded(expanded === job.id ? null : job.id)} onApply={setApplyRole} />)}
      </div>
      {filtered.length === 0 && <p data-testid="jobs-empty" className="text-center font-body text-[#666666] py-12">{c.none}</p>}
      <ApplyModal open={applyRole !== null} onClose={() => setApplyRole(null)} role={applyRole || ''} lang={lang} />
    </div>
  );
}

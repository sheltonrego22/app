...
const h2 = "font-heading font-black text-3xl sm:text-4xl text-[#EEEDE7] uppercase tracking-tight mb-3";

function SectionHeading({ visible, title, sub }) {
  return (
    <div className={`text-center mb-14 ${visible ? 'scroll-visible' : 'scroll-hidden'}`}>
      <div className="orange-accent-line mx-auto mb-6" />
      <h2 className={h2}>{title}</h2>
      {sub && <p className="font-body text-[#666] max-w-xl mx-auto">{sub}</p>}
    </div>
  );
}
  ...
  return (
    <section data-testid="used-cars-hero" className="relative min-h-[70vh] flex items-center overflow-hidden">
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 pt-28 pb-16 text-center">
        ...
        <h1 className="font-heading font-black text-4xl sm:text-6xl lg:text-7xl text-[#EEEDE7] uppercase tracking-tight mb-4 animate-fade-in-up">Pre-Owned Vehicles You Can Trust. Backed by 50 Years of Fleet Expertise.</h1>
...
export function AuctionCycleBar() {
  const cell = (label, value) => (
    <div>
      <p className="font-heading text-[10px] text-[#666] uppercase tracking-wider">{label}</p>
      <p className="font-heading font-bold text-sm text-[#EEEDE7]">{value}</p>
    </div>
  );
  return (
  ...
  return (
    <section className="bg-[#0a0a0a] py-20 sm:py-28">
      <div ref={ref} className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <SectionHeading visible={visible} title="This Week's Stock" />
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {sampleVehicles.map((v, idx) => (
            <div key={v.name} className={`bg-[#111] border border-white/5 p-5 hover:border-[#EE5A01]/30 transition-all ${reveal} stagger-${(idx % 4) + 1}`}>
              ...
              <div className="border-t border-white/5 pt-3 flex items-center justify-between">
                ...
                <Link to="/contact" className="text-[#EE5A01] font-heading text-xs uppercase tracking-wider hover:underline flex items-center gap-1">Enquire <ArrowRight className="w-3 h-3" /></Link>
              </div>
            </div>
          ))}
        </div>
...
export function WhyBuy() {
  ...
  return (
    <section className="bg-black py-20 sm:py-28">
      <div ref={ref} className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <SectionHeading visible={visible} title="Not Just Used. Ex-Eurogulf-Managed." sub="Every vehicle in the silent auction came directly from the Europcar Dubai operating-lease fleet. We know every kilometre, every service, and every owner from day one." />
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          {advantages.map((a, i) => (
            <div key={a.title} className={`bg-[#111] border border-white/5 p-7 hover:border-[#EE5A01]/30 transition-all ${reveal} stagger-${i + 1}`}>
  ...
  return (
    <section id="how-it-works" className="bg-[#0a0a0a] py-20 sm:py-28">
      <div ref={ref} className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <SectionHeading visible={visible} title="How the Auction Works" sub="Wholesale-grade rigour. Retail-grade simplicity." />
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {auctionSteps.map((s, i) => (
            <div key={s.num} className={`text-center ${reveal} stagger-${i + 1}`}>
              <div className="w-12 h-12 bg-[#EE5A01] flex items-center justify-center mx-auto mb-4"><span className="font-heading font-black text-lg text-black">{s.num}</span></div>

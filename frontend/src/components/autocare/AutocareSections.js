...
function SectionHeading({ visible, children }) {
  return (
    <div className={`text-center mb-14 ${visible ? 'scroll-visible' : 'scroll-hidden'}`}>
  ...
  return (
    <section data-testid="autocare-hero" className="relative min-h-[70vh] flex items-center overflow-hidden bg-black">
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 pt-28 pb-16 text-center">
        ...
        <h1 className="font-heading font-black text-4xl sm:text-6xl lg:text-7xl text-[#EEEDE7] uppercase tracking-tight mb-4 animate-fade-in-up">Eurogulf Auto Garage</h1>
  ...
  return (
    <section className="bg-black py-20 sm:py-28">
      <div ref={ref} className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <SectionHeading visible={visible}>Our Services</SectionHeading>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {services.map((s, i) => (
            <div key={s.title} data-testid={`autocare-service-${i}`} className={`bg-[#111] border border-white/5 p-7 hover:border-[#EE5A01]/30 transition-all ${reveal} stagger-${i + 1}`}>
...
export function FeaturesGrid() {
  ...
  return (
    <section className="bg-[#0a0a0a] py-20 sm:py-28">
      <div ref={ref} className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <SectionHeading visible={visible}>Why Eurogulf Auto Garage</SectionHeading>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {features.map((f, i) => (
            <div key={f.title} data-testid={`autocare-feature-${i}`} className={`bg-[#111] border border-white/5 p-6 text-center ${reveal} stagger-${i + 1}`}>
      ...
      <div className="max-w-5xl mx-auto px-4 text-center">
        ...
        <div className="flex flex-wrap items-center justify-center gap-6">
          {insurancePartners.map((p) => (
            <div key={p} className="bg-[#111] border border-white/5 px-6 py-3"><span className="font-heading font-bold text-sm text-[#EEEDE7]">{p}</span></div>
          ))}
        </div>
      </div>
    </section>
...
export function ReceptionAndCta() {
  const linkCls = "flex items-center gap-2 text-sm text-[#666] hover:text-[#EE5A01] transition-colors font-body";
  return (
    <>
      <section className="bg-[#f5f2ec] py-16">
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <p className="font-body text-[#666] mb-6">Our service centres feature comfortable customer reception areas with refreshments while you wait. Alternatively, we offer vehicle collection and delivery services for your convenience.</p>
          <div className="flex flex-wrap gap-3 justify-center">
            <a href="tel:800364" className={linkCls}><Phone className="w-4 h-4" /> 800 364</a>
            <a href="https://wa.me/97145063030" target="_blank" rel="noopener noreferrer" className={linkCls}><MessageSquare className="w-4 h-4" /> WhatsApp</a>
            <a href="mailto:autocare@eurogulf.ae" className={linkCls}><Mail className="w-4 h-4" /> Email</a>
          </div>
        </div>
      </section>
      <section className="bg-[#EE5A01] py-16">
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <p className="font-body text-black/70 mb-8 max-w-lg mx-auto">Whether you need routine maintenance or comprehensive repairs, Eurogulf Auto Garage delivers A-grade workshop quality.</p>
          <Link to="/contact" className="bg-black text-[#EEEDE7] font-heading font-bold text-sm tracking-[0.05em] px-8 py-4 hover:bg-[#111] transition-colors inline-block">Contact Us</Link>
        </div>
      </section>
    </>
  );

            ...
            <div className="flex flex-col items-center gap-2 relative">
              <div
                ...
                className={`w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center transition-all duration-300 ${
                    ...
                    : active
                    ? 'bg-[#EE5A01]/20 border-2 border-[#EE5A01] text-[#EE5A01]'
                    : 'bg-[#111111] border border-[#333] text-[#666]'
                }`}
              >
                {done ? <Check className="w-5 h-5" /> : <Icon className="w-5 h-5" />}
              </div>
              <span className={`text-[10px] sm:text-xs font-heading tracking-wider uppercase whitespace-nowrap ${
                active ? 'text-[#EE5A01]' : done ? 'text-[#EEEDE7]' : 'text-[#666]'
              }`}>
                {s.label}
              </span>
            </div>
            {i < steps.length - 1 && (
              <div className={`h-[2px] flex-1 mx-2 sm:mx-3 transition-colors duration-300 ${
                done ? 'bg-[#EE5A01]' : 'bg-[#222]'
              }`} />
            )}
          </div>
        );
...
function VehicleCard({ vehicle, duration, selected, onSelect }) {
